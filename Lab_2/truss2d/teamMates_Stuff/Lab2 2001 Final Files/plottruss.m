function plottruss(joints,connectivity,barforces,reacjoints)
...
minfrc=floor(min(barforces));
maxfrc=ceil(max(barforces));
...
for i=1:numelem
    na=connectivity(i,1);
    nb=connectivity(i,2);
    [xc,yc,zc]=plotbar(joints(na,:),joints(nb,:),radb);
    cc=barforces(i)*ones(size(xc));
    surf(xc,yc,zc,cc,'EdgeColor','none');
   % if pltflags(2) > 0
   %     text((joints(na,1)+joints(nb,1))/2+1.8*radb,(joints(na,2)+joints(nb,2))/2+1.8*radb,(joints(na,3)+joints(nb,3))/2+1.8*radb,num2str(i));
   % end
   % if pltflags(3) > 0
   %     text((joints(na,1)+joints(nb,1))/2+1.8*radb,(joints(na,2)+joints(nb,2))/2+1.8*radb,(joints(na,3)+joints(nb,3))/2+1.8*radb,sprintf('%.2f',barforces(i)));
   % end
    hold on;
end
...
%if pltflags(1) > 0
%    for i=1:numnode
%            text(joints(i,1)+1.8*radj,joints(i,2)+1.5*radj,joints(i,3)+1.3*radj,num2str(i));
%    end
%end
...
for i=1:numnode
    [sx,sy,sz]=plotnode(joints(i,:),radj);
...
for i=1:length(reacjoints)
    na=reacjoints(i);
    [sx,sy,sz]=plotnode(joints(na,:),radk);
